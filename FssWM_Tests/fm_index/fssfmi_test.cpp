...
void FssFMI_Offline_Test() {
    ...
    std::vector<FssFMIParameters> params_list = {
        FssFMIParameters(10, 10, 3, fsswm::fss::OutputType::kSingleBitMask),
        FssFMIParameters(10, 10, 3, fsswm::fss::OutputType::kShiftedAdditive),
        // FssFMIParameters(10),
        // FssFMIParameters(15),
        // FssFMIParameters(20),
    };
    ...
    for (const auto &params : params_list) {
        ...
        if (params.GetFssWMParameters().GetOSParameters().GetParameters().GetOutputType() == fsswm::fss::OutputType::kSingleBitMask) {
            // Save keys
            std::string key_path = kTestFssFMIPath + "fssfmikeySBM_d" + ToString(d);
            ...
            // Save data
            std::string db_path    = kTestFssFMIPath + "dbSBM_d" + ToString(d);
            ...
            // Save keys
            std::string key_path = kTestFssFMIPath + "fssfmikeySA_d" + ToString(d);
            key_io.SaveKey(key_path + "_0", keys[0]);
            key_io.SaveKey(key_path + "_1", keys[1]);
            key_io.SaveKey(key_path + "_2", keys[2]);
            ...
            // Save data
            std::string db_path    = kTestFssFMIPath + "dbSA_d" + ToString(d);
            std::string query_path = kTestFssFMIPath + "query_d" + ToString(d);
...
void FssFMI_SingleBitMask_Online_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "FssFMI_SingleBitMask_Online_Test...");
    std::vector<FssFMIParameters> params_list = {
        FssFMIParameters(10, 10, 3, fsswm::fss::OutputType::kSingleBitMask),
        // FssFMIParameters(10),
        // FssFMIParameters(15),
    ...
    for (const auto &params : params_list) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=, &result](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                brss.OnlineSetUp(party_id, kTestFssFMIPath + "prf");

                // Evaluate the longest-prefix-match operation
                RepShareVec64 result_sh(qs);
                eval.EvaluateLPM_SingleBitMask(chls, key, db_sh, query_sh, result_sh);
    ...
    Logger::DebugLog(LOC, "FssFMI_SingleBitMask_Online_Test - Passed");
}

void FssFMI_ShiftedAdditive_Online_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "FssFMI_ShiftedAdditive_Online_Test...");
    std::vector<FssFMIParameters> params_list = {
        FssFMIParameters(10, 10, 3, fsswm::fss::OutputType::kShiftedAdditive),
        // FssFMIParameters(10),
        // FssFMIParameters(15),
    ...
    Logger::DebugLog(LOC, "FssFMI_ShiftedAdditive_Online_Test - Passed");
}
