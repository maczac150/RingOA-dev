...
void OblivSelect_Online_Test(const oc::CLP &cmd) {
    Logger::DebugLog(LOC, "OblivSelect_Online_Test...");
    std::vector<OblivSelectParameters> params_list = {
        OblivSelectParameters(5),
        // OblivSelectParameters(10),
        // OblivSelectParameters(15),
    ...
    for (const auto &params : params_list) {
        ...
        auto task_p0 = [&](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
            ReplicatedSharing3P  rss(d);
            OblivSelectEvaluator eval(params, rss);
            ...
            eval.Evaluate(chls, uv_prev, uv_next, key, database_sh, index_sh, result_sh);
        ...
        if (result != database[index])
            throw oc::UnitTestFail("OblivSelect_Online_Test failed: result = " + std::to_string(result) +
                                   ", expected = " + std::to_string(database[index]));
    }
    Logger::DebugLog(LOC, "OblivSelect_Online_Test - Passed");
}
