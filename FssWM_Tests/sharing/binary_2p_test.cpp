...
void Binary2P_EvaluateXor_Offline_Test() {
    ...
    for (const uint32_t bitsize : kBitsizes) {
        ...
        // Save input
        std::string x_path = kTestBinaryPath + "x_n" + std::to_string(bitsize);
        std::string y_path = kTestBinaryPath + "y_n" + std::to_string(bitsize);
...
void Binary2P_EvaluateXor_Online_Test() {
    ...
    for (const uint32_t bitsize : kBitsizes) {
        ...
        // Validate the result
        if (z != (5 ^ 4) || z_arr != std::array<uint32_t, 2>{(1 ^ 5), (2 ^ 4)} || z_vec != std::vector<uint32_t>{(1 ^ 5), (2 ^ 4), (3 ^ 3), (4 ^ 2), (5 ^ 1)})
            throw oc::UnitTestFail("EvaluateXor failed.");
...
void Binary2P_EvaluateAnd_Offline_Test() {
    ...
    for (const uint32_t bitsize : kBitsizes) {
        ...
        std::string triple_path = kTestBinaryPath + "triple_n" + std::to_string(bitsize);
...
void Binary2P_EvaluateAnd_Online_Test() {
    ...
    for (const uint32_t bitsize : kBitsizes) {
        ...
        // Validate the result
        if (z != (4 & 5) || z_arr != std::array<uint32_t, 2>{(1 & 5), (2 & 4)})
            throw oc::UnitTestFail("EvaluateAnd failed.");
    }
