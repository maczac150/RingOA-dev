...
void Additive2P_EvaluateMult_Online_Test() {
    ...
    for (const uint64_t bitsize : kBitsizes) {
        ...
        // Validate the result
        if (z != 20 || z_arr != std::array<uint64_t, 2>{5, 8})
...
void Additive2P_EvaluateSelect_Offline_Test() {
    ...
    for (const uint64_t bitsize : kBitsizes) {
        ...
        uint64_t                c     = 1;
        std::array<uint64_t, 2> c_arr = {1, 0};

        // Generate conditional shares
        std::pair<uint64_t, uint64_t>                               c_sh     = ss.Share(c);
        std::pair<std::array<uint64_t, 2>, std::array<uint64_t, 2>> c_arr_sh = ss.Share(c_arr);

        Logger::DebugLog(LOC, "c: " + ToString(c));
        Logger::DebugLog(LOC, "c_0: " + ToString(c_sh.first) + ", c_1: " + ToString(c_sh.second));
...
void Additive2P_EvaluateSelect_Online_Test() {
    ...
    for (const uint64_t bitsize : kBitsizes) {
        ...
        // c = [1, 0, 1, 0, 1]
        // z = [5, 2, 3, 4, 1]
        if (z != 4 || z_arr != std::array<uint64_t, 2>{5, 2})
