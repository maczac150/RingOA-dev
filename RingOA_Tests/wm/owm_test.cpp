...
using ringoa::wm::OWMFscEvaluator;
using ringoa::wm::OWMFscKey;
using ringoa::wm::OWMFscKeyGenerator;
using ringoa::wm::OWMFscParameters;
...
void OWM_Fsc_Offline_Test() {
    Logger::DebugLog(LOC, "OWM_Fsc_Offline_Test...");
    std::vector<OWMFscParameters> params_list = {
        OWMFscParameters(10),
        // OWMFscParameters(15),
        // OWMFscParameters(20),
    };
    ...
    for (const auto &params : params_list) {
        ...
        AdditiveSharing2P   ass(d);
        ReplicatedSharing3P rss(d);
        OWMFscKeyGenerator  gen(params, ass, rss);
        FileIo              file_io;
        ShareIo             sh_io;
        KeyIo               key_io;
        ...
        // Generate keys
        std::array<OWMFscKey, 3> keys = gen.GenerateKeys(v_sign);

        // Save keys
        std::string key_path = kTestOWMPath + "owmfsckey_d" + ToString(d);
        key_io.SaveKey(key_path + "_0", keys[0]);
        key_io.SaveKey(key_path + "_1", keys[1]);
        key_io.SaveKey(key_path + "_2", keys[2]);

        // Save data
        std::string db_path       = kTestOWMPath + "dbfsc_d" + ToString(d);
        std::string aux_path      = kTestOWMPath + "auxfsc_d" + ToString(d);
        std::string query_path    = kTestOWMPath + "queryfsc_d" + ToString(d);
        std::string position_path = kTestOWMPath + "positionfsc_d" + ToString(d);
    ...
    Logger::DebugLog(LOC, "OWM_Fsc_Offline_Test - Passed");
}

void OWM_Fsc_Online_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "OWM_Fsc_Online_Test...");
    std::vector<OWMFscParameters> params_list = {
        OWMFscParameters(10),
        // OWMFscParameters(15),
        // OWMFscParameters(20),
    ...
    for (const auto &params : params_list) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=, &result](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                ReplicatedSharing3P rss(d);
                AdditiveSharing2P   ass_prev(d), ass_next(d);
                OWMFscEvaluator     eval(params, rss, ass_prev, ass_next);
                Channels            chls(party_id, chl_prev, chl_next);

                // Load this party's key
                OWMFscKey key(party_id, params);
                KeyIo     key_io_local;
                key_io_local.LoadKey(key_path + "_" + ToString(party_id), key);
                ...
                RepShare64                 result_sh;
                std::vector<ringoa::block> uv_prev(1U << nu), uv_next(1U << nu);
                eval.EvaluateRankCF(chls, key, uv_prev, uv_next, db_sh, RepShareView64(aux_sh), RepShareView64(query_sh), position_sh, result_sh);
        ...
        if (result != expected_result) {
            throw osuCrypto::UnitTestFail(
                "OWM_Fsc_Online_Test failed: result = " + ToString(result) +
                ", expected = " + ToString(expected_result));
        }
    }

    Logger::DebugLog(LOC, "OWM_Fsc_Online_Test - Passed");
}
