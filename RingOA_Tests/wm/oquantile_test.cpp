    ...
    for (const auto &params : params_list) {
        ...
        if (result != expected_result) {
            throw osuCrypto::UnitTestFail(
                "OQuantile_Online_Test failed: result = " + ToString(result) +
...
void OQuantile_Fsc_Offline_Test() {
    Logger::DebugLog(LOC, "OQuantile_Fsc_Offline_Test...");
    std::vector<OQuantileFscParameters> params_list = {
        OQuantileFscParameters(10, 7),
        // OQuantileFscParameters(15),
        // OQuantileFscParameters(20),
    };
    ...
    for (const auto &params : params_list) {
        ...
        // Save keys
        std::string key_path = kTestOQuantilePath + "oquantilefsckey_d" + ToString(d);
        key_io.SaveKey(key_path + "_0", keys[0]);
        key_io.SaveKey(key_path + "_1", keys[1]);
        key_io.SaveKey(key_path + "_2", keys[2]);

        // Save data
        std::string db_path    = kTestOQuantilePath + "dbfsc_d" + ToString(d);
        std::string aux_path   = kTestOQuantilePath + "auxfsc_d" + ToString(d);
        std::string q_arg_path = kTestOQuantilePath + "queryfsc_d" + ToString(d);
    ...
    Logger::DebugLog(LOC, "OQuantile_Fsc_Offline_Test - Passed");
}

void OQuantile_Fsc_Online_Test(const osuCrypto::CLP &cmd) {
    Logger::DebugLog(LOC, "OQuantile_Fsc_Online_Test...");
    std::vector<OQuantileFscParameters> params_list = {
        OQuantileFscParameters(10, 7),
        // OQuantileFscParameters(15),
        // OQuantileFscParameters(20),
    ...
    for (const auto &params : params_list) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=, &result](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                // Load this party's key
                OQuantileFscKey key(party_id, params);
                KeyIo           key_io_local;
                key_io_local.LoadKey(key_path + "_" + ToString(party_id), key);
                ...
                ShareIo       sh_io;
                sh_io.LoadShare(db_path + "_" + ToString(party_id), db_sh);
                sh_io.LoadShare(aux_path + "_" + ToString(party_id), aux_sh);
                sh_io.LoadShare(q_arg_path + "_" + ToString(party_id), q_arg_sh);
        ...
        if (result != expected_result) {
            throw osuCrypto::UnitTestFail(
                "OQuantile_Fsc_Online_Test failed: result = " + ToString(result) +
                ", expected = " + ToString(expected_result));
        }
    }

    Logger::DebugLog(LOC, "OQuantile_Fsc_Online_Test - Passed");
}
