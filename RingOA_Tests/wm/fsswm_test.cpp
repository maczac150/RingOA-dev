...
void FssWM_Offline_Test() {
    ...
    for (const auto &params : params_list) {
        ...
        FssWMKeyGenerator   gen(params, ass, rss);
        FileIo              file_io;
        ShareIo             sh_io;
        KeyIo               key_io;
        ...
        // Save data
        std::string db_path       = kTestFssWMPath + "db_d" + ToString(d);
    ...
    for (const auto &params : params_list) {
        ...
        auto MakeTask = [&](int party_id) {
            return [=, &result](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
                ...
                FssWMEvaluator      eval(params, rss, ass_prev, ass_next);
