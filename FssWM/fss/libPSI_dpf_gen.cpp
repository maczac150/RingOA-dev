...
BgiPirClient::uint128_t BgiPirClient::bytesToUint128_t(const span<u8> &data) {
    if (data.size() > 16)
        throw std::runtime_error("inputsize is too large, 128 bit max. " LOCATION);
...
void BgiPirClient::keyGen(uint128_t idx, block seed, span<block> k0, span<block> g0, span<block> k1, span<block> g1) {
    ...
    for (u64 i = 0; i < u64(g0.size()); ++i) {
        ...
        // std::cout << "s0[" << i << "] " << stt(s0[i]) << std::endl;
        // std::cout << "s1[" << i << "] " << stt(s1[i]) << std::endl;
    }
    ...
    for (u64 i = 0; i < u64(g0.size()); ++i) {
        gs0[i] = (gs0[i] ^ s0[i]);
        gs1[i] = (gs1[i] ^ s1[i]);
        // std::cout << "gs0[" << i << "] " << gs0[i] << " = G(" << s0[i] << ")" << std::endl;
        // std::cout << "gs1[" << i << "] " << gs1[i] << " = G(" << s1[i] << ")" << std::endl;
    }
    for (u64 i = 0; i < u64(g0.size()); ++i) {
        gs0[i] = gs0[i] ^ gs1[i];
    }
...
void BgiPirClient::keyGenSingleGroup(uint128_t idx, block seed, span<block> k0, span<block> g0, span<block> k1, span<block> g1) {
    static const block notOneBlock   = toBlock(~0, ~1);
    static const block notThreeBlock = toBlock(~0, ~3);
    ...
    std::vector<block> temp(4);
    auto               s0  = temp.data();
    auto               s1  = temp.data() + 1;
    auto               gs0 = temp.data() + 2;
    auto               gs1 = temp.data() + 3;

    s0[0] = s[0] & notThreeBlock;
    s1[0] = s[1] & notThreeBlock;
    std::cout << "s0 " << stt(s0[0]) << std::endl;
    std::cout << "s1 " << stt(s1[0]) << std::endl;

    aes0.ecbEncBlocks(s0, 2, gs0);
    // gs0 = AES_enc(s0), gs1 = AES_enc(s1)

    gs0[0] = gs0[0] ^ s0[0];
    gs1[0] = gs1[0] ^ s1[0];
    gs0[0] = gs0[0] ^ gs1[0];
    std::cout << "gs0 " << gs0[0] << std::endl;

    u64  byteIdx = gIdx % 16 + 16 * (gIdx / 128);
    u64  bitIdx  = (gIdx % 128) / 16;
    auto u8View  = reinterpret_cast<u8 *>(gs0);
    u8View[byteIdx] ^= static_cast<u8>(1) << bitIdx;

    memcpy(g0.data(), u8View, sizeof(block));
    memcpy(g1.data(), u8View, sizeof(block));
