#ifndef SHARING_BINARY_2P_H_
#define SHARING_BINARY_2P_H_
...
public:
    BinarySharing2P() = delete;
    explicit BinarySharing2P(const uint32_t bitsize);
    ...
    void EvaluateXor(const uint32_t &x, const uint32_t &y, uint32_t &z) const;
    void EvaluateXor(const std::array<uint32_t, 2> &x, const std::array<uint32_t, 2> &y, std::array<uint32_t, 2> &z) const;
    void EvaluateXor(const std::vector<uint32_t> &x, const std::vector<uint32_t> &y, std::vector<uint32_t> &z) const;

    void EvaluateAnd(const uint32_t party_id, osuCrypto::Channel &chl, const uint32_t &x, const uint32_t &y, uint32_t &z);
    void EvaluateAnd(const uint32_t party_id, osuCrypto::Channel &chl, const std::array<uint32_t, 2> &x, const std::array<uint32_t, 2> &y, std::array<uint32_t, 2> &z);
    // void EvaluateAnd(const uint32_t party_id, osuCrypto::Channel &chl, const std::vector<uint32_t> &x, const std::vector<uint32_t> &y, std::vector<uint32_t> &z);
