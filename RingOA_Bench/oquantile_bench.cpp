...
#include "RingOA/wm/oquantile_fsc.h"
...
using ringoa::wm::OQuantileFscEvaluator;
using ringoa::wm::OQuantileFscKey;
using ringoa::wm::OQuantileFscKeyGenerator;
using ringoa::wm::OQuantileFscParameters;
...
void OQuantile_VAF_Online_Bench(const osuCrypto::CLP &cmd) {
    ...
    auto MakeTask = [&](int p) {
        ...
        return [=](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
            ...
            const std::string timer_eval_name  = "OQuantile VAF Eval " + ptag;
            const int32_t     timer_setup      = timer_mgr.CreateNewTimer(timer_setup_name);
            const int32_t     timer_eval       = timer_mgr.CreateNewTimer(timer_eval_name);
...
void OQuantile_Fsc_VAF_Offline_Bench(const osuCrypto::CLP &cmd) {
    ...
    Logger::InfoLog(LOC, "OQuantile (FSC) VAF Offline Benchmark started (repeat=" + ToString(repeat) + ")");

    // VAF database size is approximately 23 million entries < 2^25
    // sigma for VAF values is 7 (0-100)
    OQuantileFscParameters params(25, 7);
    params.PrintParameters();
    ...
    AdditiveSharing2P        ass(s);
    ReplicatedSharing3P      rss(s);
    OQuantileFscKeyGenerator gen(params, ass, rss);
    FileIo                   file_io;
    ShareIo                  sh_io;
    KeyIo                    key_io;
    TimerManager             timer_mgr;

    std::string key_path    = kBenchWmPath + "oquantilefsckey_d" + ToString(d);
    std::string db_path     = kBenchWmPath + "dbfsc_vaf_d" + ToString(d);
    std::string aux_path    = kBenchWmPath + "auxfsc_vaf_d" + ToString(d);
    std::string query1_path = kBenchWmPath + "queryfsc_brca1_d" + ToString(d);
    std::string query2_path = kBenchWmPath + "queryfsc_brca2_d" + ToString(d);

    // 1) Data generation + secret sharing (once per d)
    std::array<bool, 3> v_sign;

    {
        const std::string timer_name = "OQuantile (FSC) VAF DataGen";
        const int32_t     timer_id   = timer_mgr.CreateNewTimer(timer_name);
        timer_mgr.SelectTimer(timer_id);
        ...
        gen.GenerateDatabaseU64Share(wm, db_sh, aux_sh, v_sign);
        timer_mgr.Mark("ShareDB d=" + ToString(d));
    ...
    // 2) KeyGen (repeat times)
    {
        const std::string timer_name = "OQuantile (FSC) VAF KeyGen";
        const int32_t     timer_id   = timer_mgr.CreateNewTimer(timer_name);
        timer_mgr.SelectTimer(timer_id);

        for (uint64_t i = 0; i < repeat; ++i) {
            timer_mgr.Start();
            std::array<OQuantileFscKey, 3> keys = gen.GenerateKeys(v_sign);
            timer_mgr.Stop("d=" + ToString(d) + " iter=" + ToString(i));
    ...
    // 3) OfflineSetUp (measured once per d)
    {
        const std::string timer_name = "OQuantile (FSC) VAF OfflineSetUp";
        const int32_t     timer_id   = timer_mgr.CreateNewTimer(timer_name);
        timer_mgr.SelectTimer(timer_id);
    ...
    Logger::InfoLog(LOC, "OQuantile (FSC) VAF Offline Benchmark completed");
    Logger::ExportLogListAndClear(kLogWmPath + "oquantile_fsc_vaf_offline_bench", /*use_timestamp=*/true);
}
...
void OQuantile_Fsc_VAF_Online_Bench(const osuCrypto::CLP &cmd) {
    ...
    Logger::InfoLog(LOC, "OQuantile (FSC) VAF Online Benchmark started (repeat=" + ToString(repeat) +
                             ", party=" + ToString(party_id) + ")");
    ...
    auto MakeTask = [&](int p) {
        ...
        return [=](osuCrypto::Channel &chl_next, osuCrypto::Channel &chl_prev) {
            ...
            // ----- Timers -----
            TimerManager      timer_mgr;
            const std::string timer_setup_name = "OQuantile (FSC) VAF OnlineSetUp " + ptag;
            const std::string timer_eval_name  = "OQuantile (FSC) VAF Eval " + ptag;
            const int32_t     timer_setup      = timer_mgr.CreateNewTimer(timer_setup_name);
            const int32_t     timer_eval       = timer_mgr.CreateNewTimer(timer_eval_name);
            ...
            ReplicatedSharing3P   rss(s);
            AdditiveSharing2P     ass_prev(s), ass_next(s);
            OQuantileFscEvaluator eval(params, rss, ass_prev, ass_next);
            Channels              chls(p, chl_prev, chl_next);
            RepShare64            result_sh;

            // Load key for this party
            OQuantileFscKey key(p, params);
            KeyIo           key_io;
            key_io.LoadKey(key_path + "_" + ToString(p), key);
            ...
            for (uint64_t i = 0; i < repeat; ++i) {
                ...
                eval.EvaluateQuantile_Parallel(
                    chls, key, uv_prev, uv_next,
                    db_sh, RepShareView64(aux_sh), left_sh, right_sh, k_sh, result_sh);
                timer_mgr.Stop("d=" + ToString(d) + " iter=" + ToString(i));
    ...
    Logger::InfoLog(LOC, "OQuantile (FSC) VAF Online Benchmark completed");
    Logger::ExportLogListAndClear(kLogWmPath + "oquantile_fsc_vaf_brca" + ToString(qid) + "_online_p" + ToString(party_id) + "_" + network,
                                  /*use_timestamp=*/false);
}
