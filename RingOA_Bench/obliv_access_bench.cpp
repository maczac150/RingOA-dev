...
//     Logger::InfoLog(LOC, "RingOa_Fsc_Offline_Bench...");
//     uint64_t iter = cmd.isSet("iter") ? cmd.get<uint64_t>("iter") : kIterDefault;
...
//         TimerManager timer_mgr;
//         int32_t      timer_keygen = timer_mgr.CreateNewTimer("RingOaFsc KeyGen");
//         int32_t      timer_off    = timer_mgr.CreateNewTimer("RingOaFsc OfflineSetUp");

//         std::string key_path = kBenchOSPath + "ringoafsckey_d" + ToString(d);
//         std::string db_path  = kBenchOSPath + "dbfsc_d" + ToString(d);
//         std::string idx_path = kBenchOSPath + "idxfsc_d" + ToString(d);
...
//     Logger::ExportLogList("./data/logs/oa/ringoa_fsc_offline_bench");
// }

// void RingOa_Fsc_Online_Bench(const osuCrypto::CLP &cmd) {
//     Logger::InfoLog(LOC, "RingOa_Fsc_Online_Bench...");
//     uint64_t    iter     = cmd.isSet("iter") ? cmd.get<uint64_t>("iter") : kIterDefault;
//     int         party_id = cmd.isSet("party") ? cmd.get<int>("party") : -1;
//     std::string network  = cmd.isSet("network") ? cmd.get<std::string>("network") : "";
...
//     Logger::InfoLog(LOC, "RingOa_Fsc_Online_Bench - Finished");
//     Logger::ExportLogList("./data/logs/oa/ringoa_fsc_online_p" + ToString(party_id) + "_" + network);
// }
