...
public:
    ...
    explicit BinarySharing2P(const uint64_t bitsize);
    ...
    void EvaluateXor(const uint64_t &x, const uint64_t &y, uint64_t &z) const;
    void EvaluateXor(const std::array<uint64_t, 2> &x, const std::array<uint64_t, 2> &y, std::array<uint64_t, 2> &z) const;
    void EvaluateXor(const std::vector<uint64_t> &x, const std::vector<uint64_t> &y, std::vector<uint64_t> &z) const;

    void EvaluateAnd(const uint64_t party_id, osuCrypto::Channel &chl, const uint64_t &x, const uint64_t &y, uint64_t &z);
    void EvaluateAnd(const uint64_t party_id, osuCrypto::Channel &chl, const std::array<uint64_t, 2> &x, const std::array<uint64_t, 2> &y, std::array<uint64_t, 2> &z);
    // void EvaluateAnd(const uint64_t party_id, osuCrypto::Channel &chl, const std::vector<uint64_t> &x, const std::vector<uint64_t> &y, std::vector<uint64_t> &z);
