...
uint64_t IntegerComparisonEvaluator::EvaluateSharedInput(osuCrypto::Channel &chl, const IntegerComparisonKey &key, const uint64_t x, const uint64_t y) const {
    ...
    Logger::DebugLog(LOC, party_str + " x: " + ToString(x) + ", y: " + ToString(y));
#endif

    if (party_id == 0) {
        // Reconstruct masked inputs
        std::array<uint64_t, 2> masked_x_0, masked_x_1, masked_x;
        ss_in_.EvaluateAdd({x, y}, {key.shr1_in, key.shr2_in}, masked_x_0);
        ...
        // Reconstruct masked inputs
        std::array<uint64_t, 2> masked_x_0, masked_x_1, masked_x;
        ss_in_.EvaluateAdd({x, y}, {key.shr1_in, key.shr2_in}, masked_x_1);
        ss_in_.Reconst(party_id, chl, masked_x_0, masked_x_1, masked_x);
...
uint64_t IntegerComparisonEvaluator::EvaluateMaskedInput(const IntegerComparisonKey &key, const uint64_t x, const uint64_t y) const {
    uint64_t party_id = key.ddcf_key.dcf_key.party_id;
    uint64_t n        = params_.GetInputBitsize();
